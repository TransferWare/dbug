...
Used to tell the runtime support module the name of the function being
entered. The arguments are a module name and an address to a long to store context for
the return. See also C<DBUG_ENTER>.

EX: long context; dbug_enter( "main", &context );

=item dbug_leave

Used at each exit point of a function containing a dbug_enter function at
the entry point. See also C<DBUG_RETURN>.

EX: dbug_leave( context );
...
To print as much as five string arguments. See also C<DBUG_PRINT>.

EX: dbug_print2( "keyword", "This is %s and that is %s", "this", "that" );
...
the C<DBUG_PUSH> macro. This section summarizes the currently
...
at this time. See also C<README.prof>.
...
match the argument passed to the C<DBUG_PROCESS> macro.
...
Used in conjunction with the C<DBUG_PUSH> macro to reset the current
indentation level back to zero. Most useful with C<DBUG_PUSH> macros
...
The C<DBUG_ENTER> macro informs that debugger that we have entered the
...
line. The C<DBUG_PROCESS> macro is generally used only once per
program to inform the debugger what name the program was invoked with.
The C<DBUG_PUSH> macro modifies the current debugger state by saving
the previous state and setting a new state based on the control string
passed as its argument. The C<DBUG_PRINT> macro is used to print the
values of each argument for which a factorial is to be computed. The
C<DBUG_RETURN> macro tells the debugger that the end of the current
...
I<dbug> runtime support routines via the C<DBUG_PUSH> macro. This
particular string enables output from

the C<DBUG_PRINT> macro with the 'd' flag and enables function tracing
...
such point requires the use of an appropriate C<DBUG_RETURN> or
C<DBUG_VOID_RETURN> macro.
...
point "<main" because at the time the C<DBUG_ENTER> macro was reached
in main, tracing was not enabled yet. It was only after the macro
C<DBUG_PUSH> was executing that tracing became enabled. This implies
that the argument list should be processed as early as possible since
all code preceding the first call to C<DBUG_PUSH> is essentially
...
to every C<DBUG_PRINT> macro is a I<dbug> keyword.
...
The second argument to a C<DBUG_PRINT> macro is a standard printf
