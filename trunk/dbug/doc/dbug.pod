...
stream. The input arguments are a line (for instance __LINE__), a break point,
a format string and the corresponding argument list. Note that all parameters
are all one macro argument and B<must> be enclosed in parenthesis. The
DBUG_PRINT_CTX has one extra input parameter: the debugging context. Before a
DBUG_PRINT statement can be executed a DBUG_ENTER statement must have been
executed, i.e. DBUG_PRINT needs a function context.

EX: DBUG_PRINT ((__LINE__, "type", "type is %x", type)); 

EX: dbug_ctx_t dbug_ctx; DBUG_PRINT_CTX ((dbug_ctx, __LINE__, "stp", "%x -> %s", stp, stp -> name));
...
comma (,) or semi-colon (;) and may not have spaces.
...
=item DBUG_INIT, DBUG_INIT_CTX, dbug_init, dbug_init_ctx

A line containing the address of dbug context, I, major version, minor version,
teeny version, name, context number, process id and the flags will be
...
DBUG#<address of dbug context>#I#<major version>.<minor version>.<teeny version>#<name>#<context number>#<process id>#<flags>

=item DBUG_DONE, DBUG_DONE_CTX, dbug_done, dbug_done_ctx

A line containing the address of the dbug context, D, the maximum number of
functions on the stack and the stack usage will be printed.

This is the line printed: 

DBUG#<dbug context>#D#<maximum number of functions on the stack>#<stack usage>

=item DBUG_ENTER, DBUG_ENTER_CTX, dbug_enter, dbug_enter_ctx

A line consisting of the address of dbug context, E, file, function, line, level
and time (-1 if profiling is off else in milliseconds) is printed.

This is the line printed: 

DBUG#<address of dbug context>#E#<file>#<function>#<line>#<level>#<time>

=item DBUG_LEAVE, DBUG_LEAVE_CTX, dbug_leave, dbug_leave_ctx

A line consisting of the address of dbug context, L, file, function, line, level
and time (-1 if profiling is off else in milliseconds) is printed.

This is the line printed: 

DBUG#<address of dbug context>#L#<file>#<function>#<line>#<level>#<time>

=item DBUG_PRINT, DBUG_PRINT_CTX, dbug_print, dbug_print_ctx

A line consisting of the address of dbug context, P, file, function, line, level,
break point and user supplied parameters is printed.

DBUG#<address of dbug context>#P#<file>#<function>#<line>#<level>#<break point>#<user supplied parameters>

=item Errors

Errors are printed to stderr in the following format:

DBUG#<address of dbug context>#ERROR: <function name>: <description>
...
comma (or semi-colon) separated list of debugger arguments.

Each debugger argument is a single character flag followed by an
optional argument specific to the given flag.
...
file name and the 'L' flag enables printing of the source file line number.
...
for determining the factorial of 3 when run with the debug control string "-#t,D1000,g".
	...
	Figure 11
 	factorial -#t,D1000,g 3 | perl -S dbugrpt
