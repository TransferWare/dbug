...
=item DBUG_INIT(options, name) and DBUG_INIT_CTX(options, name, dbug_ctx)
...
=item DBUG_DONE() and DBUG_DONE_CTX(dbug_ctx)
...
=item DBUG_ENTER(function) and DBUG_ENTER_CTX(dbug_ctx, function)
...
=item DBUG_LEAVE() and DBUG_LEAVE_CTX(dbug_ctx)
...
=item DBUG_PRINT(break_point, arglist) and DBUG_PRINT_CTX(dbug_ctx, break_point, arglist)
